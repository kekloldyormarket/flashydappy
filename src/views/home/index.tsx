...
: Promise<any> {
	...
	for (const tokenAddress of mrgnFiTokenAddresses) {
      ...
      const liab = marginfiAccount.getBalance(bank.address).computeQuantity(bank).liabilities.integerValue().toNumber();
		  ...
		  assets[token.symbol] = ass / 10 ** bank.mintDecimals
		  liabs[token.symbol] = liab / 10 ** bank.mintDecimals
		}
	  }
	}
	return {assets, liabs};
 ...
 function projectActiveBalancesNoCpi(program: any, instructions: TransactionInstruction[], account: any): PublicKey[] {
  let projectedBalances = [...account.balances];
  ...
  return projectedBalances.filter((b) => b.active).map((b) => b.bankPk);
   ...
   async function makeBeginFlashLoanIx(endIndex: number,marginfiAccount: MarginfiAccountWrapper, program: any): Promise<any> {
    return marginfiAccount.makeBeginFlashLoanIx(endIndex);
  }

   async function makeEndFlashLoanIx(projectedActiveBalances: PublicKey[], marginfiAccount: MarginfiAccountWrapper, program: any, client: any): Promise<any> {
    return marginfiAccount.makeEndFlashLoanIx(projectedActiveBalances);
  }
    ...
    <div className="md:hero mx-auto p-4">
      <div className="md:hero-content flex flex-col">
        ...
        {error && <div style={{color: "red"}}>{error}</div>}
				{loading ? <div>Loading...this may take a min or two</div> 
        :
        <div> 
